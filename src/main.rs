...
async fn main() -> std::io::Result<()> {
    ...
    let db = Db::new(&config.mongo_uri, &config.database_name, &config.collection_name)
        .await
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .route("/insert", web::post().to(handlers::handlers::insert_dataset))
            .service(
                web::resource("/get/{id}").route(web::get().to(handlers::handlers::get_dataset)),
            )
            .service(
                web::resource("/list").route(web::get().to(handlers::handlers::list_datasets)),
            )
    })
    .bind("127.0.0.1:8080")?
    .run()
