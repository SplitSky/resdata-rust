...
BASE_URL="http://127.0.0.1:8080/insert"

test_insert_dataset() {
	local url="${BASE_URL}/insert"
	local data='{"name": "Test", "description": "Test description"}'
	local response=$(curl -s -o /dev/null -w "%(http_code)" -X POST "$curl" -H "Content-Type: application/json" -d "$data")
